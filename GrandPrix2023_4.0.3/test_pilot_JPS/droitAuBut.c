...
int nodeEquals(Node *node1, Node *node2) {
    ...
    return node1->x == node2->x && node1->y == node2->y && node1->speedX == node2->speedX &&
           node1->speedY == node2->speedY;
}
...
double heuristicCost(Node *a, Node *b, int speedX, int speedY, char **map) {
    ...
    double combined_heuristic = 1 * heuristic1 + 0.5 * heuristic2; /* Change the weights as you see fit*/
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            if (map[y][x] == '=') {
                ...
                arrayEnd->array = (EndPosition *) realloc(arrayEnd->array,
                                                          sizeof(EndPosition) * (endPositionCount + 1));
            }
        }
    }
...
findBestEnd(int myX, int myY, int secondX, int secondY, int thirdX, int thirdY, int speedX, int speedY, ArrayEnd *array,
    ...
    *end = createNode(array->array[0].x, array->array[0].y, NULL, speedX, speedY, 0);
    ...
    for (j = 0; j < array->size; j++) {
        fprintf(stderr, "endPositions[%d] = (%d, %d, %f)\n", j, array->array[j].x, array->array[j].y,
                array->array[j].distance);
    }
