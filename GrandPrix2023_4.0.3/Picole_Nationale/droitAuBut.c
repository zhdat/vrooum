...
Position dijkstra_next_move(char** map, int width, int height, Position start,
    ...
    while (1) {
        ...
        if (min_node.cost == INF ||
            (min_node.pos.x == end.x && min_node.pos.y == end.y)) {
            break;
        }
    ...
    if (dist[end.y][end.x] == INF) {
        ...
        while (!(prev[current.y][current.x].x == start.x &&
                 prev[current.y][current.x].y == start.y)) {
...
int main() {
    ...
    while (!feof(stdin)) {
        ...
        fflush(stderr);
        fgets(line_buffer, MAX_LINE_LENGTH,
              stdin); /* Read positions of pilots */
        sscanf(line_buffer, "%d %d %d %d %d %d", &myX, &myY, &secondX, &secondY,
               &thirdX, &thirdY);
        fprintf(stderr, "    Positions: Me(%d,%d)  A(%d,%d), B(%d,%d)\n", myX,
                myY, secondX, secondY, thirdX, thirdY);
        fflush(stderr);
        /* Gas consumption cannot be accurate here. */
        gasLevel +=
            gasConsumption(accelerationX, accelerationY, speedX, speedY, 0);
        ...
        fflush(stderr);
        if (0 &&
            round > 4) { /* (DISABLED) Force a segfault for testing purpose */
            int* p = NULL;
